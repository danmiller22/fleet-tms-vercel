...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === "PUT") {
      ...
      let existing = [];
      if (keys.length) {
        const { data: ex, error: exErr } = await supa
          .from("tms")
          .select("pk,data")
          .eq("table_name", table)
          .in("pk", keys);
      ...
      const upserts = payload.map(row => {
        const pk = primaryKeyOf(row);
        const prev = map.get(pk) || {};
        const merged = Object.assign({}, prev, row);
        return { table_name: table, pk, data: merged, updated_at: new Date().toISOString() };
      });

      const { error: upErr } = await supa
        .from("tms")
        .upsert(upserts, { onConflict: "table_name,pk" });
