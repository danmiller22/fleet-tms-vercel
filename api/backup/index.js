...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === "POST") {
      ...
      const tables = ["trucks", "trailers", "expenses", "repairs", "invoices"];
      const rows = [];
      ...
      for (const t of tables) {
        ...
        rows.push({
          key: `backup:${ts}:tms:${t}`,
          table_name: t,
          payload: (data || []).map(r => r.data),
          created_at: new Date().toISOString()
        });
      }

      const { error: upErr } = await supa.from("backups").upsert(rows, { onConflict: "key" });
    ...
    if (req.method === "GET") {
      ...
      return res.status(200).send(JSON.stringify((data && data.payload) || []));
    }
